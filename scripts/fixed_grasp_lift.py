...
def cup_grasp_target(cup_position, *, rim_x_offset, grasp_y_offset, grasp_z_offset=0.0):
    return (cup_position[0] + rim_x_offset, cup_position[1] + grasp_y_offset,
...
def object_grasp_target(object_position, *, x_offset, y_offset, z_offset):
    return (object_position[0] + x_offset, object_position[1] + y_offset,
...
def _save_rgb_frame(annotator, frames_dir, index):
    ...
    if data is None:
        return False
    Image.fromarray(data[:, :, :3].astype(np.uint8)).save(frames_dir / f"rgb_{index:04d}.png")
    return True
...
def _encode_compact_gif(frames_dir, output_path):
    ...
    images[0].save(output_path, save_all=True, append_images=images[1:],
                   duration=round(1000 / VERIFY_VIDEO_FPS), loop=0, optimize=True)
...
def patch_gripper_friction(stage, friction=2.0):
    ...
    print(f"GRIPPER_PATCH: applied friction={friction} to {len(finger_prim_paths)} finger prims", flush=True)
...
def high_friction_collision(stage, friction=1.5):
    ...
    for prim in Usd.PrimRange(stage.GetPrimAtPath("/World")):
        if prim.HasAPI(UsdPhysics.CollisionAPI):
            ...
            binding.Bind(material)
            count += 1
    print(f"FRICTION_PATCH: applied friction={friction} to {count} collision prims", flush=True)
...
def parse_args():
    p = argparse.ArgumentParser(description="Fixed grasp+lift with spine-first lift + high friction.")
    p.add_argument("--object-name", choices=("cup", "bowl2", "spoon2", "plate2", "simple_tray"), default="cup")
    ...
    p.add_argument("--friction", type=float, default=2.0, help="Physics friction coefficient for gripper/cup")
    p.add_argument("--spine-only-lift", action="store_true", default=True,
                   help="Lift using spine only (keep arm joints fixed)")
    p.add_argument("--public-ip", default=os.environ.get("PUBLIC_IP"))
    p.add_argument("--out-dir", type=Path, default=REPO_ROOT / "outputs" / "fixed_grasp_lift")
    return p.parse_args()
...
def main():
    ...
    simulation_app = _prepare_sim(args.record_video, args.livestream, args.public_ip)
...
def _verify(args, simulation_app, frames_dir):
    ...
    sim = SimulationContext(sim_utils.SimulationCfg(dt=0.005, device="cuda:0", gravity=(0.0, 0.0, -9.81)))

    spawn_position = (ROTATE_SPOT[0], ROTATE_SPOT[1], 0.1117) if args.skip_navigation else (0.2961, 1.572, 0.1117)
    ...
    from isaaclab_assets.robots.fr3 import _prepare_rigid_body_view_path as prepare_rigid_body_view_path
    ...
    scene = InteractiveScene(
        make_control_scene_cfg(
            num_envs=1,
            robot_path=make_headless_robot_usd(REPO_ROOT / "assets" / "mobile_fr3_duo_v0_2.usd"),
            robot_position=spawn_position,
    ...
    object_view = RigidPrim(prim_paths_expr=object_path, name=f"task3_{args.object_name}")
    ...
    if args.record_video:
        camera = rep.create.camera(position=CAMERA_POSITION, look_at=CAMERA_LOOK_AT)
    ...
    def sim_tick():
        ...
        if base_hold_anchor is not None:
            ...
            vx, vy = _btt(adapter.pose(), base_hold_anchor, max_linear_mps=0.25, position_kp=4.0)
    ...
    def log_phase(name, ok, **detail):
        ...
        entry = {"phase": name, "ok": bool(ok), "tick": tick_count,
                 ...
                 "spine": round(arms.measured_spine_position(), 3), **detail}
        phases.append(entry)
    ...
    def servo_arm(side, position, quat, *, budget_s, tol_m=0.02):
        try:
            return arms.reach(side, position, quat, step=sim_tick, dt=sim.cfg.dt,
                              timeout_s=budget_s, position_tolerance_m=tol_m)
        except ValueError:
            return False

    # === Phase 0: raise spine, tuck ===
    travel_spine_target = TRAVEL_SPINE_M
    spine_ok = arms.move_spine(travel_spine_target, step=sim_tick, dt=sim.cfg.dt,
                               timeout_s=6.0, tolerance_m=0.02)
    log_phase("raise_spine", spine_ok, target_spine=travel_spine_target)
    if not spine_ok:
        return _result(False, "raise_spine", cup_position(), cup_position(), phases, args, frames_dir,
                       frames_written, rgb_annotator, render_product, sim)
    ...
    if not ok:
        return _result(False, "navigation", cup_start, cup_position(), phases, args, frames_dir,
                       frames_written, rgb_annotator, render_product, sim)
    ...
    if not ok:
        return _result(False, "pregrasp", cup_start, cup_position(), phases, args, frames_dir,
                       frames_written, rgb_annotator, render_product, sim)

    # === Phase 3: descend ===
    cup_before_descend = cup_position()
    grasp = cup_grasp_target(cup_before_descend, rim_x_offset=CUP_RIM_X_OFFSET,
                              grasp_y_offset=CUP_GRASP_Y_OFFSET,
                              grasp_z_offset=args.cup_grasp_z_offset)
    strict_reach = servo_arm("right", grasp, top_down, budget_s=6.0, tol_m=0.015)
    ...
    log_phase("descend", ok, strict_reach=strict_reach,
              position_error_m=round(final_approach_error, 4),
              target=[round(v, 3) for v in grasp])
    if not ok:
        return _result(False, "descend", cup_start, cup_position(), phases, args, frames_dir,
                       frames_written, rgb_annotator, render_product, sim)

    # Re-center on live cup after descend contact push
    live_cup = cup_position()
    grasp = cup_grasp_target(live_cup, rim_x_offset=CUP_RIM_X_OFFSET,
                              grasp_y_offset=CUP_GRASP_Y_OFFSET,
                              grasp_z_offset=args.cup_grasp_z_offset)
    servo_arm("right", grasp, top_down, budget_s=4.0, tol_m=0.02)
    log_phase("recenter_live_cup", arms.position_error("right", grasp) <= 0.10,
              position_error_m=round(arms.position_error("right", grasp), 4))

    # Close gripper with increased effort
    holding = arms.grasp("right", step=sim_tick, dt=sim.cfg.dt,
    ...
    log_phase("close", holding, gripper_position_rad=round(gripper_position, 4))
    if not holding:
        return _result(False, "close", cup_start, cup_position(), phases, args, frames_dir,
                       frames_written, rgb_annotator, render_product, sim)
    ...
    if args.spine_only_lift:
        ...
        if not lift_ok:
            ...
            if remaining > 0.01:
                ...
                arm_lift_target = (post_spine_pose[0][0], post_spine_pose[0][1],
                ...
                for tick in range(math.ceil(3.0 / sim.cfg.dt)):
                    ...
                    arms.set_arm_target("right", (post_spine_pose[0][0], post_spine_pose[0][1], target_z),
                                        post_spine_pose[1])
                    arms.command()
                    sim_tick()
                current_cup = cup_position()
        ...
        lift_ok = arms.lift("right", dz, step=sim_tick, dt=sim.cfg.dt, timeout_s=6.0,
                            position_tolerance_m=0.03, spine_assist_m=0.12)
    ...
    passed = grasp_lift_gate_passed(holding=holding, held_ticks=held_ticks,
                                     needed_ticks=needed_ticks, lifted_m=lifted,
                                     min_lift_m=args.min_lift_m)
    log_phase("hold", passed, lifted_m=round(lifted, 4),
    ...
    return _result(passed, final_phase, cup_start, cup_end, phases, args, frames_dir,
                   frames_written, rgb_annotator, render_product, sim,
                   continuous_hold_seconds=max_held_ticks * sim.cfg.dt)


def _result(passed, failed_phase, cup_start, cup_end, phases, args, frames_dir, frames_written,
            rgb_annotator, render_product, sim, continuous_hold_seconds=0.0):
    if args.record_video and rgb_annotator is not None:
        ...
        if frames_written:
            _encode_compact_gif(frames_dir, frames_dir.parent / "grasp_lift.gif")
    return {
        "passed": bool(passed),
