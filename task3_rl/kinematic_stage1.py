...
class KinematicStage1Env(VecEnv):
    ...
    def __init__(
        self,
        cfg: KinematicStage1Cfg = KinematicStage1Cfg(),
        device: str = "cuda:0",
    ) -> None:
        self.cfg = cfg
        ...
        self._goal_xy = torch.tensor(
            self._task_cfg.goal_xy, device=self.device
        ...
        self._tray_velocity = torch.zeros(
    ...
    def step(
        ...
        if actions.shape != (self.num_envs, self.num_actions):
            ...
            raise ValueError(
                f"Expected actions {expected}, got {tuple(actions.shape)}"
            )
        ...
        extras: dict[str, torch.Tensor | dict[str, torch.Tensor]] = {
            ...
            "log": {
                ...
                "/metrics/mean_distance": torch.linalg.vector_norm(
                    self._tray_pose[:, :2] - self._goal_xy, dim=-1
                ).mean(),
            },
        }
        if done_ids.numel() > 0:
            extras["log"]["/metrics/episode_return"] = self._episode_return[
                done_ids
            ].mean()
    ...
    def _reset(self, env_ids: torch.Tensor) -> None:
        count = env_ids.numel()
        jitter = (
            torch.rand((count, 2), device=self.device) * 2.0 - 1.0
        ) * self.cfg.start_jitter_m
        start_xy = (
            torch.tensor(self.cfg.start_xy, device=self.device).expand(
                count, -1
            )
            + jitter
        )
